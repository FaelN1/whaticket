...
const ScheduleModal = ({ open, onClose, scheduleId, contactId, cleanContact, reload }) => {
	...
	const handleSaveSchedule = async (data) => {
		try {
			...
			// Criar um agendamento para cada contato selecionado
			for (const contact of selectedContacts) {
				const scheduleData = {
					...data,
					contactId: contact.id,
					whatsappId: selectedWhatsapps,
					ticketUserId: selectedUser ? selectedUser.id : null,
					queueId: selectedQueue,
					intervalo: intervalo,
					tipoDias: tipoDias,
					assinar: data.assinar
				};
				...
				if (scheduleId) {
					...
					const { data: newSchedule } = await api.post("/schedules", scheduleData);
					if (attachment != null) {
						const formData = new FormData();
						formData.append("file", attachment);
						await api.post(`/schedules/${newSchedule.id}/media-upload`, formData);
					}
				}
			}
			
			toast.success(selectedContacts.length > 1 
				? `Agendamento criado para ${selectedContacts.length} contatos com sucesso!` 
				: i18n.t("scheduleModal.success"));
								...
								<div className={classes.multFieldLine}>
									...
									>
										<Autocomplete
											...
											options={contacts}
											onChange={(e, newContacts) => {
												setSelectedContacts(newContacts);
												// Se houver pelo menos um contato selecionado, use o primeiro para definir o canal
												if (newContacts && newContacts.length > 0) {
													setChannelFilter(newContacts[0].channel || "whatsapp");
												} else {
											...
											renderInput={(params) => <TextField {...params} variant="outlined" placeholder="Selecione um ou mais contatos" />}
										/>
									</FormControl>
								</div>
								<div className={classes.multFieldLine}>
