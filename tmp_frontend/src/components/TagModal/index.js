...
const TagModal = ({ open, onClose, tagId, kanban }) => {
	...
	const handleSaveTag = async values => {
		// Garantir que o campo position seja incluído nos dados enviados ao backend
		const tagData = { 
			...values, 
			userId: user?.id, 
			kanban: kanban, 
			...
			position: values.position || 0 // Incluir explicitamente o campo position
		};
										...
										<Grid item xs={12} md={3} xl={3}>
											<Field
												...
												onChange={(e) => {
													...
													if (value >= 0 && value <= 15) {
														setTag(prev => ({ ...prev, position: value }));
													}
												}}
												fullWidth
											/>
