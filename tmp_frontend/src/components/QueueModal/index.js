...
const QueueModal = ({ open, onClose, queueId, onEdit }) => {
  ...
  useEffect(() => {
    if (activeStep && queue?.chatbots && queue.chatbots[activeStep]) {
  ...
  useEffect(() => {
    if (inputRefs.current.length > 0 && queue.chatbots.length > 0) {
      const ref = inputRefs.current[queue.chatbots.length - 1];
      if (ref && typeof ref.focus === 'function') ref.focus();
    }
  }, [queue.chatbots.length]);

  const handleExportFlux = () => {
    const dataStr = JSON.stringify(queue.chatbots, null, 2);
    const blob = new Blob([dataStr], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `fluxo-chatbot-${queue.name || 'fila'}.json`;
    ...
    reader.onload = (e) => {
      try {
        ...
        if (Array.isArray(imported)) {
          setQueue(prev => ({ ...prev, chatbots: imported }));
          toast.success('Fluxo importado com sucesso!');
        } else {
          toastError('Arquivo inválido. O JSON deve ser um array de etapas.');
        }
      } catch (err) {
        toastError('Erro ao importar o fluxo. Verifique o arquivo JSON.');
      }
    };
                ...
                <DialogContent dividers>
                  ...
                  <Field
                    ...
                    onFocus={() => {
                      ...
                      if (greetingRef && greetingRef.current) {
                      ...
                      >
                        ...
                        >
                          ...
                          {integrations && integrations.length > 0 ? integrations.map((integration) => (
                            ...
                            <MenuItem value={""}>Nenhuma integração disponível</MenuItem>
                          )}
                        </Field>
                    ...
                    >
                      ...
                      >
                        ...
                        {file && file.length > 0 ? file.map(f => (
                          ...
                          <MenuItem value={""}>Nenhum arquivo disponível</MenuItem>
                        )}
                      </Field>
                    </FormControl>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
                    ...
                    <input
                      ...
                      id="import-flux-json"
                      type="file"
                      onChange={handleImportFlux}
                    />
                    <label htmlFor="import-flux-json">
                      <Button
                        variant="outlined"
                        color="primary"
                        component="span"
                    ...
                    {({ push, remove }) => (
                      <>
                        <Typography variant="subtitle1" style={{marginBottom: 12}}>
                          Monte o fluxo do chatbot abaixo.
                        </Typography>
                        <Stepper
                          nonLinear
                        ...
                        >
                          {values.chatbots &&
                            ...
                            values.chatbots.map((info, index) => (
                              ...
                              >
                                <StepLabel key={`${info?.id ? info.id : index}-chatbots`}>
                                  {isNameEdit !== index &&
                                    queue?.chatbots && 
                                ...
                                {isStepContent && queue?.chatbots && queue.chatbots[index] && (
                                  <StepContent>
                                    <>
                                      {isGreetingMessageEdit !== index ? (
                                        ...
                                        >
                                          ...
                                          {queue?.chatbots && !queue.chatbots[index]
                                            ?.greetingMessage && (
                                              <CustomToolTip
                          ...
                          <Button color="primary" variant="contained" onClick={push} style={{marginTop: 10, fontWeight: 700, boxShadow: '0 2px 6px #1976d233'}}>
                            + Adicionar opção
                          </Button>
                          {values.chatbots.length === 0 && (
                            <Typography variant="body2" color="textSecondary" style={{marginTop: 18}}>
                              Nenhuma etapa adicionada ainda. Clique em <b>Adicionar opção</b> para começar seu fluxo.
                            </Typography>
                          )}
                        </>
                      )}
