...
const CampaignModal = ({
  ...
  useEffect(() => {
    if (isMounted.current) {
      ...
      api.get(`/campaigns/${campaignId}`).then(({ data }) => {
        ...
        // Verifica se a campanha é recorrente pelo nome
        const recurrenceMatch = data.name.match(/\[RECORRENTE:(\d+)\]$/);
        const isRecurrent = !!recurrenceMatch;
        const recurrenceInterval = isRecurrent ? parseInt(recurrenceMatch[1], 10) : 30;
        
        // Remove o marcador de recorrência do nome para exibição
        const cleanName = data.name.replace(/\s*\[RECORRENTE:\d+\]\s*$/, '').trim();
        ...
        setCampaign((prev) => {
          ...
          Object.entries(data).forEach(([key, value]) => {
            if (key === "scheduledAt" && value !== "" && value !== null) {
              ...
              // Usa o nome limpo, sem o marcador de recorrência
              prevCampaignData[key] = cleanName;
            } else {
              prevCampaignData[key] = value === null ? "" : value;
            }
          });
          
          // Adiciona os campos de controle de recorrência
          prevCampaignData.isRecurrent = isRecurrent;
          prevCampaignData.recurrenceInterval = recurrenceInterval;
  ...
  const handleSaveCampaign = async (values) => {
    try {
      ...
      // Se a campanha for recorrente, adiciona a informação no nome
      if (values.isRecurrent && values.recurrenceInterval > 0) {
        // Formato: Nome Original [RECORRENTE:30]
        // Onde 30 é o intervalo em dias
        const originalName = values.name.replace(/\s*\[RECORRENTE:\d+\]\s*$/, '').trim();
        dataValues.name = `${originalName} [RECORRENTE:${values.recurrenceInterval}]`;
      } else {
        // Remove o marcador de recorrência se existir
        dataValues.name = values.name.replace(/\s*\[RECORRENTE:\d+\]\s*$/, '').trim();
      }
      ...
      delete dataValues.isRecurrent;
      delete dataValues.recurrenceInterval;
      delete dataValues.recurrenceCount;
      delete dataValues.recurrenceEndDate;
      delete dataValues.recurrenceExecuted;
                ...
                <Grid spacing={2} container>
                  ...
                  <Grid xs={12} md={4} item>
                    <FormControl
                      ...
                      className={classes.formControl}
                    >
                      <InputLabel id="isRecurrent-selection-label">
                        Repetir Campanha
                      </InputLabel>
                      <Field
                        as={Select}
                        label="Repetir Campanha"
                        labelId="isRecurrent-selection-label"
                        id="isRecurrent"
                        name="isRecurrent"
                        error={
                          touched.isRecurrent && Boolean(errors.isRecurrent)
                        }
                        disabled={!campaignEditable}
                      >
                    ...
                    <>
                      <Grid xs={12} md={4} item>
                        <Field
                          ...
                          helperText={touched.recurrenceInterval && errors.recurrenceInterval || "Intervalo em dias para repetição"}
                          variant="outlined"
                          margin="dense"
                          type="number"
                          InputLabelProps={{
                      ...
                      <Grid xs={12} md={4} item>
                        <Field
                          ...
                          error={touched.recurrenceCount && Boolean(errors.recurrenceCount)}
                          helperText={touched.recurrenceCount && errors.recurrenceCount || "0 = repetir indefinidamente"}
                          variant="outlined"
                          margin="dense"
                          type="number"
                          InputLabelProps={{
                      ...
                      <Grid xs={12} md={4} item>
                        <Field
                          ...
                          helperText={touched.recurrenceEndDate && errors.recurrenceEndDate || "Opcional - Data limite para repetições"}
                          variant="outlined"
                          margin="dense"
                          type="date"
                          InputLabelProps={{
